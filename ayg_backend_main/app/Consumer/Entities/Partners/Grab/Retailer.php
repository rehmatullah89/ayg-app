...
use App\Consumer\Entities\Partners\Grab\RetailerCategoryList;
...
class Retailer extends Entity
{
    ...
    /**
     * @return \App\Consumer\Entities\Partners\Grab\RetailerCategoryList
     */
    public function getCategories(): \App\Consumer\Entities\Partners\Grab\RetailerCategoryList
    {
    ...
    public function asArrayForCsvLine(bool $forceOpeningValues): array
    {
        if ($this->getRetailerCustomization() === null || $this->getRetailerCustomization()->getVerified() !== true) {
            ...
            if ($forceOpeningValues){
                ...
                $arrayCustomize['isActive'] = $this->retailerCustomization->isIsActive() === true ? 'Y' : ($this->isStoreOnline() ? 'Y' : 'N');
            }else{
                $arrayCustomize['hasDelivery'] = $this->retailerCustomization->isHasDelivery() === false ? 'N' : ($this->isBStoreDelivery() ? 'Y' : 'N');
                $arrayCustomize['hasPickup'] = $this->retailerCustomization->isHasPickup() === false ? 'N' : ($this->isBPickupEnabled() ? 'Y' : 'N');
                $arrayCustomize['isActive'] = $this->retailerCustomization->isIsActive() === false ? 'N' : ($this->isStoreOnline() ? 'Y' : 'N');
            }
        }
