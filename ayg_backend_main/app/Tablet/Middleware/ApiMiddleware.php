...
use App\Tablet\Errors\ApiAuthError;
use App\Tablet\Errors\ErrorPrefix;
use App\Tablet\Errors\LoginFailureAppAccessError;
...
class ApiMiddleware
{
    public static function apiAuthWithoutSession(\Slim\Route $route)
    {
        ...
        $error_array = validateAPIKey($params['apikey'], $params['epoch'], 0, false, 't');
    ...
    public static function apiAuth(\Slim\Route $route)
    {
        ...
        // Validate if API Key is good, if so return the Parse Object Id
        // last parameter = t means that this uses api key salt for tablet app
        $error_array = validateAPIKey($params['apikey'], $params['epoch'], $params['sessionToken'], true, 't');
        ...
        if ($user->isHasTabletPOSAccess() !== true) {
