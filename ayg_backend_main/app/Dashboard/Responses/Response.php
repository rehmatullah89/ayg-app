...
namespace App\Dashboard\Responses;

use App\Dashboard\Errors\Error;
use App\Dashboard\Errors\ErrorPrefix;
use App\Dashboard\Errors\OtherApplicationError;
use App\Dashboard\Helpers\ConfigHelper;
use App\Dashboard\Exceptions\Exception;
...
class Response implements \JsonSerializable
{
    ...
    public function setErrorFromException($errorCodePrefix, \Exception $exception)
    {
        ...
        if (class_exists($errorName) && $errorName !== Error::class) {
            ...
            json_error(ErrorPrefix::APPLICATION_DELIVERY . ErrorPrefix::CONTROLLER_ORDER . $errorName::CODE, $errorName::MESSAGE, $exception->getMessage(), '', 1);
            return $this;
        }

        $this->error = new OtherApplicationError($errorCodePrefix, $backtraceArray);
        json_error(ErrorPrefix::APPLICATION_DELIVERY . ErrorPrefix::CONTROLLER_ORDER . OtherApplicationError::CODE, OtherApplicationError::MESSAGE, $exception->getMessage(), '', 1);
        return $this;
    }
    ...
    public function jsonSerialize()
    {
        $return = new \stdClass();
        if ($this->data !== null) {
            $return->data = json_decode(json_encode($this->data));
