...
class OAuthConsumerCredential {
    ...
    function __contruct($oauth_consumer_key, $oauth_consumer_secret, $oauth_token_or_requestor_id='', $oauth_token_secret='') {
...
class TripIt {
    ...
    function _do_request($verb, $entity=null, $url_args=null, $post_args=null) {
        if (in_array($verb, array('/oauth/request_token', '/oauth/access_token') )) {
            #$base_url = implode('/', array($this->_api_url, $verb)); // Original
			$base_url = implode('', array($this->_api_url, $verb));
        } else if($verb == 'list_air') {
			// Added
			$base_url = implode('/', array($this->_api_url, 'v1/list/object/trip_id/' . $url_args . '/type/air/format/json'));
        } else if($verb == 'list_trip') {
			// Added
			$base_url = implode('/', array($this->_api_url, 'v1/list/trip/format/json'));
		} else {
            if ($entity) {
        ...
        $args = null;
        if ($url_args && $verb != 'list_air') { // Modified
            $args = $url_args;
            $pairs = array();
    ...
    function _parse_command($func_name, $url_args = null, $post_args = null) {
        ...
        $verb = $pieces[0]; // get
        $entity = count_like_php5($pieces) > 1 ? $pieces[1] : null; // air

        $response = $this->_do_request($verb, $entity, $url_args, $post_args);
        return $this->_xml_to_php($response);        
    }

    function list_air($id) {
		
		$response = $this->_do_request('list_air', null, $id, null);
		return $response;
    ...
    function list_trip() {
		...
		$response = $this->_do_request('list_trip', null, null, null);
		return $response;
