...
while(1>0) {
    ...
    $worker9001Status = getCacheAPI9001WorkerQueueSlackNotification();


    if(!empty($mainApi9001Status)) {
        if (empty($worker9001Status)){
            setCacheAPI9001WorkerQueueSlackNotification();
        }
    ...
    if (empty($mainApi9001Status) && !empty($worker9001Status)){
        delCacheAPI9001WorkerQueueSlackNotification();
	...
	if(!isset($workerQueue)) {
		// Connect to Queue
		try {
			$workerQueue = new WorkerQueue($GLOBALS['env_workerQueueSlackNotificationConsumerName'], $GLOBALS['env_workerQueueConsumerLPInSecs']);
			$runningTime = time();
		}
		catch (Exception $ex) {
	...
	*/
    ...
    $worker9001Status = getCacheAPI9001WorkerQueuePushAndSms();
    if (!empty($worker9001Status)){
