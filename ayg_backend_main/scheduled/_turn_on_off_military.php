...
foreach ($airportIataCodes as $airportIataCode){
    $posList = new \Parse\ParseQuery('RetailerPOSConfig');
    $posList->startsWith('comments',$airportIataCode);
    $posList=$posList->find();

    foreach ($posList as $pos){
        echo $pos->get('comments');
        echo PHP_EOL;
        $pos->set('disallowEmpDiscount',false);
        $pos->save();
