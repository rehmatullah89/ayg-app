...
$itemsQuery = new ParseQuery("Retailers");
$itemsQuery->equalTo('airportIataCode', $airportIataCode);
$itemsQuery->equalTo('isActive', true);
$itemsQuery->limit(10000000);
$items = $itemsQuery->find(true);

foreach ($items as $item) {
    echo $item->get('retailerName').PHP_EOL;
    echo $item->get('uniqueId').PHP_EOL;


    delCacheByKey('__MENULOADER__RETAILERS_MD5_'.$item->get('uniqueId'));
}
