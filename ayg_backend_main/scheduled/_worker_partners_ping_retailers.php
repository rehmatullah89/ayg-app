...
while (1) {
    if (\App\Background\Helpers\LooperHelper::hasLooperLastRunTimeLimitPassed(
        ...
        \App\Background\Helpers\ConfigHelper::get('env_PingPartnerRetailerIntervalInSecs_Grab') / 60)
    ) {
        // grab section
        $startTime = new DateTime('now', new DateTimeZone('UTC'));
        echo 'grab retailers ping setter started ' . $startTime->format('c') . PHP_EOL;

        $integrationService->pingRetailers('grab');

        $endTime = new DateTime('now', new DateTimeZone('UTC'));
        echo 'grab retailers ping setter ended ' . $endTime->format('c') . PHP_EOL;
