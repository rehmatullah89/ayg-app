...
while(1>0) {
    ...
    $worker9001Status = getCacheAPI9001WorkerQueueEmail();


    if(!empty($mainApi9001Status)) {
        if (empty($worker9001Status)){
            setCacheAPI9001WorkerQueueEmail();
        }
    ...
    if (empty($mainApi9001Status) && !empty($worker9001Status)){
        delCacheAPI9001WorkerQueueEmail();
	...
	if(!isset($workerQueue)) {
		// Connect to Queue
		try {
			$workerQueue = new WorkerQueue($GLOBALS['env_workerQueueEmailConsumerName'], $GLOBALS['env_workerQueueConsumerLPInSecs']);
			$runningTime = time();
		}
		catch (Exception $ex) {
