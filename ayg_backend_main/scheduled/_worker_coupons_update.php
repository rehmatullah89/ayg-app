...
$slackService = new \App\Background\Services\SlackService($GLOBALS['env_coupons_edit_notification_slack_webhook_url']);
$couponsUpdateService = new \App\Background\Services\CouponsUpdateService(
    $s3Service,
...
while (1 > 0) {
    ...
    if (empty($mainApi9001Status) && !empty($worker9001Status)){
        delCacheAPI9001CouponsUpdate();
        echo 'api 9001 empty, cleaned worker 9001 as well'.PHP_EOL;
    }


    if (!isset($workerQueue)) {
        $workerQueue = new WorkerQueue($GLOBALS['env_RabbitMQConsumerCouponsEdit'],
    ...
    if (isset($messages[0])) {
        ...
        if ($message['action'] == 'coupons_update') {
            try {
                $couponsUpdateService->update();
                $workerQueue->deleteMessage($workerQueue->getMessageId($processMessage), $processMessage);
            } catch (Exception $exception) {
