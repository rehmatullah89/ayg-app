...
while(1>0) {
    ...
    $worker9001Status = getCacheAPI9001WorkerQueue();
	...
	if(!isset($workerQueue)) {
		...
		try {
			...
			$workerQueue = new WorkerQueue($GLOBALS['env_workerQueueConsumerName'], $GLOBALS['env_workerQueueConsumerLPInSecs']);
			$runningTime = time();
		}
		catch (Exception $ex) {
	...
	*/
    ...
    $worker9001Status = getCacheAPI9001WorkerQueueMidPriorityAsynch();
    if (!empty($worker9001Status)){
	...
	foreach($result as $processMessage) {
		...
		setQueueWorkerStartTimestamp('_worker_queue_');
		...
		if(function_exists($queueFunctionPre)) {
			...
			try {
				...
				if(is_array($queueFunctionResponse)) {
                    ...
                    delQueueWorkerStartTimestamp('_worker_queue_');
		...
		if(isset($message["processAfter"]["timestamp"]) && !$workerQueue->isMessageReadyToBeProcessed(floatval($message["processAfter"]["timestamp"]))) {
