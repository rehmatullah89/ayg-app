...
while(1>0) {
    ...
    if(!empty($mainApi9001Status)) {
        if (empty($worker9001Status)){
            setCacheAPI9001WorkerQueueMidPriorityAsynch();
        }
    ...
    if (empty($mainApi9001Status) && !empty($worker9001Status)){
        delCacheAPI9001WorkerQueueMidPriorityAsynch();
	...
	if(!isset($workerQueue)) {
		// Connect to Queue
		try {
			$workerQueue = new WorkerQueue($GLOBALS['env_workerQueueMidPriorityAsynchConsumerName'], $GLOBALS['env_workerQueueConsumerLPInSecs']);
			$runningTime = time();
		}
		catch (Exception $ex) {
